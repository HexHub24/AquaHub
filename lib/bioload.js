...
// One "stocking unit" = bioload of one adult neon tetra (mass-based,
// length^2.5 with diet/activity/body adjustments). Capacity is in
// stocking units per gallon.
//
// Calibration: a typical "comfortable" 20-gallon community tank
// (6 cardinal tetras + 4 corys, total ~27 units) lands at ~54% of
// a 50-unit capacity, which matches real-world stocking intuition.
export const STOCKING_FACTORS = {
  freshwater: 2.5,   // baseline — derived from realistic stocking levels
  planted:    2.8,   // plants buffer ammonia, modest bonus
  saltwater:  1.5,   // marine fish: bigger bodies + more sensitivity = lower limit
  reef:       1.3,   // corals add water-chemistry stability requirements
};
...
export function livestockBioload(item) {
  ...
  // We assume an average small community fish (~2.5") so we don't return zero —
  // zero would let users add unlimited customs and think they're fine.
  return {
    units: 3.0 * qty,
    perIndividual: 3.0,
    source: 'unknown',
    reason: 'Custom entry — using a conservative default (≈ a 2.5" community fish).',
  };
}

/**
 * Fall-back estimator when curated bioload_score is missing.
 * Uses the same mass-based model as the curated values, so
 * estimated and curated entries are comparable.
...
function estimateBioloadScore(species, item) {
  ...
  // Mass scales as length^2.5, anchored at Neon Tetra (1.5") = 1.0
  ...
  // Taxonomy discounts (matching the seed methodology)
  if (species.species_type === 'invertebrate') score *= 0.07;   // shrimp/snail average
  if (species.species_type === 'coral') score *= 0.05;
